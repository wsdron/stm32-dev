...
uint8_t TPAD_Init(void)
{
    ...
    else
    {
        return 1; // 失败
        printf(
            "the default button capacitor charge time is beyond expectation\n");
        printf("please recalibrate TPAD_DEFAULT_VAL_MIN and "
               "TPAD_DEFAULT_VAL_MAX\n");
    }
}
...
uint8_t TPAD_calibrate_charge_time_when_pressed(void)
{
    ...
    else
    {
        return 1; // 失败
        printf(
            "the pressed button capacitor charge time is beyond expectation\n");
